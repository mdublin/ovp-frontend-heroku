...
# creating feed URL with user_tag collected in /proctected view function
# so could be http://127.0.0.1:8000/feed/boxing
# that first feed url with videos is technically page one, but we don't see
# the page value until page 2 (when user clicks next button)
# then we will see url like: http://127.0.0.1:8000/feed/boxing?page=2
# But these urls are created via url_for() functions attached to the prev and next
# buttons in the videofeed.html template
# The url_for() functions attached to the previous and next buttons are 
# essentially creation a loop wherein the user clicks, for example, the next button,
# then url_for() generates the url for main.feed, passing in the user_tag and
# page number (that is either incremented or decremented, for next and previous)
# Due to Flask architecture, when the client GETs the url generated by url_for()
# for main.feed, this feed() view function is called (with the tag passed back in)
# and the page object in feed() gets the current page number in the url (tag?page=2)
# (((apparently even though we have hard-coded in ('page', 1), reqiest.args.get() does
# get the current value for the page argument in the URl...)))
# The new page number accessed by page object in feed(), together with the RESULTS_PER_PAGE 
# value are passed to video_feed_parser for the start, stop values in our slice operation on the 
# feed API url.
